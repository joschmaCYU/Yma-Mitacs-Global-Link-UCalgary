...
class LidarRecorder:
    def __init__(self):
        ...
        # 50Hz = 1 frame toutes les 0.02 secondes
        self.record_seconds = 10
        self.max_frames = 50 * self.record_seconds

        # Abonnement au topic généré par notre extracteur RL
        self.topic_name = "/rl/elevation_grid"
        rospy.Subscriber(self.topic_name, Float32MultiArray, self.callback)

        rospy.loginfo(f"En attente des données sur {self.topic_name}...")
        while self.latest_data is None and not rospy.is_shutdown():
            rospy.sleep(0.1)

        rospy.loginfo(
            f"Début de l'enregistrement ({self.record_seconds} secondes à 50Hz)..."
        )
    ...
    def callback(self, msg):
        # Met à jour la dernière grille reçue (187 valeurs flottantes)
    ...
    def timer_callback(self, event):
        if len(self.recorded_data) < self.max_frames:
            ...
            # Arrêt propre pour éviter les doublons d'écriture
            self.timer.shutdown()
            self.save_to_csv()
    ...
    def save_to_csv(self):
        ...
        with open(filename, mode="w", newline="") as f:
            writer = csv.writer(f)
            # En-tête : C0, C1... jusqu'à C186 (exactement les 187 points de la grille 17x11)
