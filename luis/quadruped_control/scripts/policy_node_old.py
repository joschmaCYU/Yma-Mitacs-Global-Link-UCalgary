...
class PolicyNodeTorch:
    ...
    def __init__(self):
        ...
        self.action_mode = (
            rospy.get_param("~action_mode", "delta").strip().lower()
        )  # delta | absolute
        self.base_for_delta = (
        ...
        if self.use_gz_links:
            rospy.Subscriber(
                "/gazebo/link_states", LinkStates, self.cb_gz, queue_size=20
            )
        ...
        if self.debug_obs:
            self.pub_obs_vec = rospy.Publisher(
                "/policy/obs_vec", Float32MultiArray, queue_size=10
            )
            self.pub_obs_break = rospy.Publisher(
                "/policy/obs_breakdown", String, queue_size=10
            )
        ...
        rospy.loginfo(
            ...
            self.use_odom,
            self.use_gz_links,
            str(self.autonomous_mode),
            str(self.policy_profile),
        )
        self.loop()
    ...
    def _load_model(self):
        if self.model_format == "onnx":
            if not _ORT_OK:
                rospy.logerr(
                    "ONNXRuntime not available. Install: sudo apt-get install python3-onnxruntime"
                )
                return
            try:
        ...
        if (
            ...
            try:
                ...
                for i, n in enumerate(names):
                    if (
                        n.endswith("::" + self.base_link_name)
                        or n == self.base_link_name
                    ):
                        idx = i
                        break
                if idx < 0:
                ...
                v_body = (
                    R.T.dot(v_world) if self.imu_use_R_transpose else R.dot(v_world)
                )
    ...
    def _get_pose_cmd_norm(self) -> np.ndarray:
        ...
        if self.invert_cmd_vy:
            vy = -vy
        arr = np.array(
            [vx / max(self.x_max_m, 1e-6), vy / max(self.y_max_m, 1e-6)],
            dtype=np.float32,
        )
        ...
        for key in self.obs_fields:
            if key not in bank:
                rospy.logwarn_throttle(
                    5.0, "obs_fields includes '%s' not implemented; using zero.", key
                )
        ...
        if isinstance(target_dim, int):
            ...
            if cur < target_dim:
                if auto_pad:
                    ...
                    rospy.logerr(
                        "Obs size=%d < target=%d y obs_auto_pad=false (perfil=%s)",
                        cur,
                        target_dim,
                        self.profile_name,
                    )
                ...
                if allow_trunc:
                    rospy.logwarn(
                        "Obs size=%d > target=%d; truncando por obs_allow_truncate=true (perfil=%s)",
                        cur,
                        target_dim,
                        self.profile_name,
                    )
                    ...
                    rospy.logerr(
                        "Obs size=%d > target=%d; ajusta obs_fields o habilita obs_allow_truncate (perfil=%s)",
                        cur,
                        target_dim,
                        self.profile_name,
                    )
        ...
        if self.debug_obs:
            try:
                ...
                breakdown = {
                    ...
                    "sizes": [int(x.shape[0]) for x in parts]
                    if isinstance(parts, list)
                    else [int(parts.shape[0])],
                ...
                if "base_lin_vel" in self.obs_fields:
                    breakdown["base_lin_vel"] = v_body.tolist()
                if "base_ang_vel" in self.obs_fields:
                    breakdown["base_ang_vel"] = w_body.tolist()
                if "gravity" in self.obs_fields:
                    breakdown["projected_gravity"] = grav.tolist()
                if "q14" in self.obs_fields:
                    breakdown["q"] = q14.tolist()
                if "dq14" in self.obs_fields:
                    breakdown["dq"] = dq14.tolist()
                if "last_actions14" in self.obs_fields:
                    breakdown["last_actions"] = last14.tolist()
    ...
    def map_model_action_to_control(self, a_model):
        ...
        if a_model.shape[0] != M:
            rospy.logwarn_throttle(
                2.0, "Model output len=%d, expected=%d. Adjusting.", a_model.shape[0], M
            )
            if a_model.shape[0] > M:
                a_model = a_model[:M]
            else:
    ...
    def clamp_and_smooth(self, q_target):
        if np.any(~np.isfinite(q_target)):
            rospy.logwarn_throttle(
                2.0, "Non-finite command. Using last filtered value."
            )
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            vx = (
                float(getattr(self.latest_cmd.linear, "x", 0.0))
                if self.latest_cmd
                else 0.0
            )
            vy = (
                float(getattr(self.latest_cmd.linear, "y", 0.0))
                if self.latest_cmd
                else 0.0
            )
            wz = (
                float(getattr(self.latest_cmd.angular, "z", 0.0))
                if self.latest_cmd
                else 0.0
            )
            ...
            rospy.loginfo_throttle(
                1.0, "POSE_CMD(raw): x=%.2f m, y=%.2f m, wz=%.2f", vx, vy, wz
            )
            try:
                if "pose_xy" in self.obs_fields:
                    ...
                    rospy.loginfo_throttle(
                        1.0,
                        "OBS pose_cmd_norm: [%.2f, %.2f, 0.00, 0.00]",
                        pose_xy[0],
                        pose_xy[1],
                    )
            except Exception:
            ...
            if (
                ...
                and self.use_cmd_vel
                and cmd_mag < self.cmd_vel_threshold
            ):
                out = (
                    q_base.copy()
                    if self.hold_mode == "neutral"
            ...
            if self._ort_session is not None:
                ...
                if exp_dim is not None and obs.shape[0] != exp_dim:
                    rospy.logerr_throttle(
                        2.0,
                        "Obs size=%d != esperado por el modelo=%d (perfil=%s). Revisa 'obs_fields'.",
                        obs.shape[0],
                        exp_dim,
                        self.policy_profile,
                    )
            ...
            rospy.loginfo_throttle(
                1.0,
                "POLICY: actions=%s",
                np.array2string(self.last_action_model, precision=3),
            )
